...
public class PilotGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#endif
        [ReadOnly] public ArchetypeChunkBufferType<PilotSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<PilotData> ghostPilotDataType;
...
#pragma warning restore 649
        ...
        public ArchetypeChunkComponentType<PilotData> ghostPilotDataType;
        public ArchetypeChunkComponentType<PilotMovementSystemData> ghostPilotMovementSystemDataType;
        public ArchetypeChunkComponentType<Rotation> ghostRotationType;
        public ArchetypeChunkComponentType<Translation> ghostTranslationType;
        public uint targetTick;
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<PilotSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<PilotData>(),
        ...
        {
            All = new []{
                ComponentType.ReadOnly<PilotSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadOnly<PredictedGhostComponent>(),
                ComponentType.ReadWrite<PilotData>(),
                ComponentType.ReadWrite<PilotMovementSystemData>(),
                ComponentType.ReadWrite<Rotation>(),
                ComponentType.ReadWrite<Translation>(),
            }
        });
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<PilotSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<PilotSnapshotData>(true),
                ...
                ghostPilotDataType = GetArchetypeChunkComponentType<PilotData>(),
                ghostPilotMovementSystemDataType = GetArchetypeChunkComponentType<PilotMovementSystemData>(),
...
public partial class PilotGhostSpawnSystem : DefaultGhostSpawnSystem<PilotSnapshotData>
{
    struct SetPredictedDefault : IJobParallelFor
    {
        [ReadOnly] public NativeArray<PilotSnapshotData> snapshots;
        ...
        {
            if (localPlayerId.Length == 1 && snapshots[index].GetPilotDataPlayerId() == localPlayerId[0].Value)
                predictionMask[index] = 1;
        }
    }
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<PilotSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
